...
const Navbar = () => {
  ...
  // Call Overflow animation on component mount
  useEffect(() => {
    // Wait a bit to ensure DOM elements are ready
    const timer = setTimeout(() => {
      animationRef.current = Overflow(".logo svg, .links a", 0.5);
    }, 100);
  ...
  useEffect(() => {    
    const handleScroll = () => {
      // Only track if scrolled for menu display logic
  ...
  return (
    <div className="w-full z-[999] select-none fixed top-0 left-0 bg-transparent">
      <div className="max-w-screen-3xl mx-auto p-5 sm:p-10 flex justify-between items-center text-white text-shadow">
        ...
        {/* Mobile menu button */}
        <div className="sm:hidden flex items-center gap-3 select-none">
          <button 
            onClick={toggleMenu}
        ...
        <div className="hidden sm:flex items-center gap-5">
          {/* Full menu items - hidden when scrolled */}
          <div className={`links overflow-hidden transition-all duration-300 ${isScrolled ? 'hidden' : 'flex items-center gap-7'}`}>
            {navItems.map((item) => (
              <a 
                className="text-sm font-regular hover:opacity-70 transition-opacity" 
                key={item.name}
                href={item.href}
                onClick={(e) => {
                  e.preventDefault();
          ...
          {/* Collapsed menu button - appears when scrolled */}
          {isScrolled ? (
            <button 
        ...
        {isMenuOpen && (
          <div className="absolute top-full left-0 w-full bg-black bg-opacity-90 backdrop-blur-sm py-4 px-5">
            <div className="flex flex-col gap-6 lg:flex-col lg:items-end sm:flex-row sm:justify-center sm:gap-10">
              {navItems.map((item) => (
                <a 
                  className="text-sm font-medium py-2 border-b text-shadow-[0_0_0.2em_#000] border-white border-opacity-10 sm:border-none hover:opacity-70 transition-opacity" 
                  key={item.name}
                  href={item.href}
                  onClick={(e) => {
                    e.preventDefault();
